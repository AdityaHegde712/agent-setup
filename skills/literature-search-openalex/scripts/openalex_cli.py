...
# ]
# [tool.uv.sources]
# science-skills-common = { path = "../../science-skills-common" }
# ///
...
try:
  ...
  _common_path = os.path.join(_skills_dir, "science-skills-common")
  if os.path.exists(_common_path) and _common_path not in sys.path:
...
def fetch_with_retry(
    ...
    try:
        ...
        if e.status_code == 404:
            ...
            logging.error("HTTP Error %s while fetching %s: %s", e.status_code, url, e)
        if exit_on_error:
            sys.exit(1)
        return None
...
def _try_fallback_locations(
    ...
    work_data = fetch_with_retry(work_url, {}, api_key=api_key, exit_on_error=False)
...
def handle_filter(args: argparse.Namespace) -> None:
    ...
    params = {
        k: getattr(args, k) for k in optional_params if getattr(args, k) is not None
    }
...
def main(argv: Sequence[str]) -> None:
    ...
    parser_resolve.add_argument("query", help="Name or part of the name to search for")
    ...
    # Get Entity Command
    parser_get = subparsers.add_parser("get", help="Get a single entity by its ID")
    ...
    # Filter Command
    parser_filter = subparsers.add_parser("filter", help="Filter and search entities")
    parser_filter.add_argument("entity_type", choices=ENTITY_TYPES)
    parser_filter.add_argument("--search", type=str, help="Full-text search query")
    ...
    parser_filter.add_argument("--group-by", type=str, help="Group results by a field")
    parser_filter.add_argument(
        "--per-page",
        type=int,
        default=DEFAULT_PER_PAGE,
        help=(f"Results per page (max {MAX_PER_PAGE}, default: {DEFAULT_PER_PAGE})"),
    )
    ...
    parser_filter.add_argument("--seed", type=int, help="Seed for random sampling")
    parser_filter.add_argument(
        "--select", type=str, help="Limit returned fields (e.g., id,title)"
    )
